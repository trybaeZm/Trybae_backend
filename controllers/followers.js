...
// [TODO: Move these below to a new service]
// given an array of usernames, returns an array of users
const getUserObjects = async (usernames) => {
  // its using sql query to get the user objects
  return new Promise((resolve, reject) => {
    const query = `SELECT * FROM users WHERE username IN (?)`;
    Model.connection.query(query, [usernames], (error, results) => {
      if (error) {
        reject(error);
      } else {
        resolve(results);
...
// using a list of Expo push tokens, send a notification to these users notifying them of an account they follow has bought a ticket
const sendNotificationToFollowers = async (followers, eventTitle, userName) => {
  let messages = [];
  console.log(followers, eventTitle, userName);
  followers.forEach((follower) => {
    if (Expo.isExpoPushToken(follower.Expo_push_token)) {
      messages.push({
        to: follower.Expo_push_token,
        sound: "default",
        title: "New Ticket Purchase ✨",
        body: `${userName} will be attending ${eventTitle} 🔥.`,
      });
    }
  });
